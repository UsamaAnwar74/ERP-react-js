...
const CrmTable = ({departments, LeadsReload}) => {
...
<div>
  {/* <button className="sm-btn" onClick={toggleForm}>Create Lead</button> */}
    ...
    {allLeadTask.map((DepartmentInfo, departmentIndex) => {
      if(DepartmentInfo.status==="Close"){

       return <tr  key={DepartmentInfo._id} value={DepartmentInfo.status}>
     {/* <DataTable
          //columns={columns}
          data={allLeadTask}
          selectableRows ={true}
       ...
       : <td class="badge rounded-pill  text-white mt-2  bg-success" >{DepartmentInfo.status}</td> }
        ...
        <div>
         ...
         >Update</button>
          ...
          ></div>
            <div className={"employee-form"}>
          <CrmForm toggleForm2={toggleForm2}
          />
      </div>
    </div>
