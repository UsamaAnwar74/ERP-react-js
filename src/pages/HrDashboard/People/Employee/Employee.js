...
const Employee = (props) => {
  ...
  async function deleteHandler(event) {
    ...
    swal({
      title: "Are you sure?",
      text: "Once deleted, you will not be able to recover this Saff data!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    })
    .then((willDelete) => {
      if (willDelete) {
        swal("Poof! Your Saff data has been deleted!", {
          icon: "success",
        });
      } else {
        swal("Your Saff data is safe!");
      }
    });
    ...
    <div>
      ...
      <div className={`employeeform-container ${activeClass ? "show" : ""}`}>
        ...
        <div className={"employee-form"}>
          <EmployeeForm toggleForm={toggleForm} />
        </div>
      </div>
