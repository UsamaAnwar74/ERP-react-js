...
const UserDetails = (props) => {
  ...
  async function deleteHandler(event) {
    ...
    swal({
      title: "Are you sure?",
      text: "Once deleted, you will not be able to recover Staff Data!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    })
  ...
  return (
    <>
      <TopNav />
      <Box padding="0px 70px" marginTop={"25px"}>

        <div className="d-flex justify-content-between">
        <div><Link style={{ color:'white', textDecoration:'none' }} to='/hr-dashboard/people' className="sm-btn">

          Back
        </Link></div>
       
       <div>
        <button onClick={toggleForm} className="sm-btn">
          Edit
        </button>
        ...
        <div className={"employee-form"}>
          <EditEmployeeForm toggleForm={toggleForm} />
        </div>
      </div>
        </div>
        

      <button onClick={deleteHandler} className="sm-btn">
          Delete
        </button>
        </div>
